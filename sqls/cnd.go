...
func (s *Cnd) Or(column string, args ...interface{}) *Cnd {
	// or
	s.Where(KeywordWrap(column)+"or = ?", args)
	return s
}
...
func (s *Cnd) Build(db *gorm.DB) *gorm.DB {
	...
	if len(s.Params) > 0 {
		for _, param := range s.Params {
			if strings.Contains(param.Query, "or") {
				ret = ret.Or(strings.ReplaceAll(param.Query, "or", ""), param.Args...)
			} else {
